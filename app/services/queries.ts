...
export function useRound({ gameId, roundId }: { gameId: string; roundId: string }) {
  return useSWR<GetRoundResponse>(`/api/games/${gameId}/rounds/${roundId}`, fetcherFinal, {
...
export function useCurses(onSuccess: (data: GetCursesResponse) => void) {
  return useSWR<GetCursesResponse>("/api/curses", fetcherFinal, { onSuccess });
}

export function useQuestions(onSuccess: (data: GetQuestionsResponse) => void) {
  return useSWR<GetQuestionsResponse>("/api/curses", fetcherFinal, { onSuccess });
}
